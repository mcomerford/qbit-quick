...
from qbitquick.config import APP_NAME, DATABASE_FILENAME

DB_PATH = os.path.join(platformdirs.user_config_dir(appname=APP_NAME, appauthor=False), DATABASE_FILENAME)
...
def print_db():
    ...
    with get_db_connection() as (conn, cur):
        cur.execute('''
            SELECT 
                rt.racing_torrent_hash AS racing_torrent_hash, 
